...
class Grid {
    ...
    UpdateGrid(ant) {
        ...
        let value = ant.rotation.delta[ant.rotation.value] 
...
var incRotation = (ant) => {
    ant.rotation.value = (ant.rotation.value + 1) % ant.rotation.delta.length
    // ant.rotation.value = (ant.rotation.value + ant.rotation.delta[1]) % 360
}
var decRotation = (ant) => {
    if(ant.rotation.value == 0 )
        ant.rotation.value = ant.rotation.delta.length - 1;
    else
        ant.rotation.value--;
}
